...
**Note December 2023**: This process has been observed to occasionally fail at the ```datalad save``` stage due to what appears to be an excessively long command string under the hood (generated by concatenating large numbers of file paths within a BIDS structure, which are long).  The recommended workaround at the moment is to process the index file in chunks of 1000 lines, and ```datalad save``` and ```datalad push --to origin``` after ingesting each chunk of files.
